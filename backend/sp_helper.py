...
コマンド:
  ...
  list_action_items  - 事務局アクションリストの一覧を返す（改修: ?idなし事務局ページのケースピッカー用）
...
def main():
    ...
    try:
        ...
        # 改修(?idなし事務局ページ対応): 事務局アクションリストの一覧を返す。
        # SharePoint左ナビから?id=無しで開く事務局ページで、新規登録時に紐付け先の案件を
        # 選択させるためのケースピッカー用データ取得。get_action_itemと同じサイト/GUID解決を使う
        elif cmd == 'list_action_items':
            action_site = os.environ.get('SP_ACTION_SITE', SITE_URL).rstrip('/')
            action_guid = os.environ.get('SP_ACTION_GUID', '')
            if action_guid:
                base_url = _list_base_url(action_site, action_guid)
